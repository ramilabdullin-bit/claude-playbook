#!/bin/bash
# remove-background.sh — Remove background from an image.
# Usage: ./remove-background.sh <image_path_or_url> [timeout]
...
"$SCRIPT_DIR/run.sh" /v1/images/remove_background "$body" "$TIMEOUT"

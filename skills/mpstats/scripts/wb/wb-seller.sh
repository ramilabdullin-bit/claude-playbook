...
if [ -z "$1" ] || [ "$1" = "--help" ]; then
  ...
  echo "  report      — products (default), categories, brands, trends, by_date, price_segmentation, subjects, keywords"
...
if [ "$REPORT" = "products" ]; then
  curl -s --location --request POST \
    "https://mpstats.io/api/analytics/v1/wb/seller/items?d1=${D1}&d2=${D2}&path=${SUPPLIER_ID}&fbs=${FBS}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json' \
    --data "{\"startRow\":0,\"endRow\":${LIMIT},\"filterModel\":{},\"sortModel\":[{\"colId\":\"revenue\",\"sort\":\"desc\"}]}"
else
  curl -s --location --request POST \
    "https://mpstats.io/api/analytics/v1/wb/seller/${REPORT}?d1=${D1}&d2=${D2}&path=${SUPPLIER_ID}&fbs=${FBS}" \
    --header "X-Mpstats-TOKEN: $TOKEN" \
    --header 'Content-Type: application/json'
fi

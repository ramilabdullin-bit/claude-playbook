...
## ЗОЛОТОЕ ПРАВИЛО (владелец сервера, 07.09.2026)

**Что-то сделал → СНАЧАЛА несколько раз проверил → ПОТОМ говоришь, что
работает.** Не «должно работать», не «в логе успешно», не на синтетике.

Правило появилось после четырёх кругов на ОДНОЙ жалобе за день. Каждый круг
я объявлял готовым то, что не проверил как следует:

| Что говорил | Чего не сделал | Чем кончилось |
|---|---|---|
| «Таймаут поднят» | не посмотрел, где реальный предел | он лежал в `.env`, а не в коде |
| «Ускорил впятеро» | замерил один раз | замер попал на прогретый портал, вживую было в 8 раз дольше |
| «PDF в эксель за 0,5с» | проверил на сгенерированном файле | на живом МХ-1 — 13 листов вместо таблицы, инструментом не воспользовались |
| «Починил nginx» | не проверил, что правка применилась | правил `sites-available`, сайт брал файл из `sites-enabled` |

Пять шагов, каждый из которых по разу спас бы день:

1. **На живом объекте** — тот самый документ, клиент, период. Синтетика
   годится только чтобы поймать очевидное ДО живого прогона.
2. **Снаружи, из точки пользователя** — тот же URL, та же сессия, тот же
   долгий запрос. Лог приложения не доказательство (см.
   [[verify-the-whole-chain-outside-in]]).
3. **Несколько раз** — первый прогон идёт по холодному или, наоборот,
   тёплому кешу; одна цифра не значит ничего.
4. **Проверить, что правка применилась** — тот ли файл, был ли рестарт, не
   перекрыто ли значение окружением.
5. **Сказать, на чём проверено.** Если на синтетике — назвать это синтетикой,
   а не выдавать за живой результат.
